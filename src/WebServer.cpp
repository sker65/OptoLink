...
void StartWebServer() { StartWebServer( false ); }
...
void HandleSetConfigMode() {
	...
	if( !mqtttopicprefix.endsWith( "/" ) )
		mqtttopicprefix.concat( "/" );
...
void HandleGetLogs() { WebServer.send( 200, "text/html", GetLogs() ); }
...
char* uptime( unsigned long milli ) {
	...
	mins -= hours * 60;
	hours -= days * 24;
	sprintf( _return, "Uptime %d days %2.2d:%2.2d:%2.2d.%3.3d", (byte)days, (byte)hours, (byte)mins, (byte)secs,
	         (int)milli );
	return _return;
}
...
void HandleGetValues() {
	...
	json.concat(",{\"name\":\"uptime\",\"value\":\"");
	json.concat(uptime(millis()));
	json.concat("\"}");
	json.concat( "]" );
